...
2. Repeat the following loop. Use `LOOP_ID=1` for the first pass and increment it for every later pass; never reset it during this task.
  ...
  b. Simulate each alpha on WorldQuant BRAIN and save the simulation settings and results in `WORKDIR`. Drop any whose self-correlation with a submitted alpha exceeds 0.85.
