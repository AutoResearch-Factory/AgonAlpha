...
这是本项目用于批量创建并轮询 BRAIN simulation、获取提交检查和提交 Alpha 的命令行客户端。所有 simulation 创建都必须使用该共享客户端，不能直接 `POST /simulations`，否则无法执行整个项目共享的并发限制。
...
- `--max-active` 默认 `3`，表示当前客户端希望同时维护的最大候选数；它不能绕过项目级硬上限。
...
所有客户端进程共享 `${CLAUDE_PLUGIN_ROOT}/alphas/.brain-simulations.json`。创建 simulation 前，客户端在文件锁内清理启动超过一小时的记录，并仅在有效记录少于代码常量 `MAX_CONCURRENT_SIMULATIONS = 3` 时登记并创建。Simulation 完成或明确失败后删除自己的 lease；达到全局上限时等待，不创建终态错误。`CONCURRENT_SIMULATION_LIMIT_EXCEEDED` 同样会重新排队，而不是写入 `error.json`。

`mcts.py discard-pending` 会在新 Dispatcher 启动时清空该 registry。`creation.json` 保存 `lease_id` 和 `global_count_active`，因此 `max-runtime` 后恢复不会重复计数。
